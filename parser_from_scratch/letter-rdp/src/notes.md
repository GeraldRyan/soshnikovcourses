...
Ok so how does precedence work? We know what is highest and lowest precedence: parentheses over all, then multiplication, and addition and then assigmnet (assuming we're not implementing powers and factorials and such). Therefore, we know they have to go lower in the tree, and therefore we konw they have to be called later, and that is by the lower precedence things. Lower precedence things get called first and then call the higher precedence things. It has to be this way because they have to be at the top. string = 'y = x + 2 * 3', and so as we parse this expression or statement, we call assignment (the lowest precedence) and that creates a left node for the literal (primary expression) y. It knows to do this because it sees an '=' in the lookahead, and the symbol 'x' is indeed valid. Then it eats the '=' because it doesn't need it anymore. Then it runs the assignment expression again (because it is the lowest precedence and hey it could be another assignment - x = y = 2). It has to be ready for that, to build that node. However, if when we called the first AssignmentExpression, it put x as the left, and it called the recursive descent procedure for the right side (after it ate the left and wrote it to the operator field). Seeing that the next expression is a plus, it calls Addition but it would have called that if it was a multiplication sign anyways because Addition is lower precedence. Let's say we have 'y = x * 2 + 3' so we see the times sign first. It stil calls addition because the plus operator has to be at a higher level in the tree. The left side of the right side will be an Addition Binary Expression. So top level we have identifier y, operator = and the first top level right side, l=0. Left side does not go deeper but right side does, and it's addition Binary Expression with a left and right. It's left will be an x*2 Binary expression with an * operator and it's right will be a literal. So this is the theory of recursion calls and precedence. It is in the geometry and architecture of a tree. Yep, that's how it works. Try to visualize. Try to understand. 
